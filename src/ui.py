...
class MainWindow(QWidget):
	...
	def create_ui(self):
		...
		self.pagelayout = PageLayout(self)
		self.pagelayout.set_name("pagelayout")
		
		self.port = RadioButton(internationalization["portrait"],self)
		self.port.setChecked(True)
		self.port.set_name("portrait")
		
		self.land = RadioButton(internationalization["landscape"], self)
		self.land.set_name("landscape")
		
		self.stretch = RadioButton(internationalization["stretch"], self)
		self.stretch.set_name("stretch")
		
		self.in_width = RadioButton(internationalization["in_width"], self)
		self.in_width.set_name("in_width")
		
		self.in_height = RadioButton(internationalization["in_height"], self)
		self.in_height.set_name("in_height")
		
		self.stretch_proportion = RadioButton(internationalization["stretch_proportion"], self)
		self.stretch_proportion.set_name("stretch_proportion")
				
		self.spinbox_number_of_pages = SpinBoxField(self)
		self.spinbox_number_of_pages.set_name("number_of_pages")
		self.spinbox_number_of_pages.setMinimum(1)
		
		self.realsize = RealSizeRadioButton(internationalization["realsize"], self)
		self.realsize.set_name("realsize")
		self.realsize.setChecked(True)
		
		self.print_all = RadioButton(internationalization["print_all"], self)
		self.print_all.set_name("print_all")
		
		self.paper = ComboBoxPaperSize(self)
		self.paper.set_name("papersize")
		
		self.printer = ComboBoxPrinter(self)
		self.printer.set_name("printer")
		
		self.rect = RectItem(self)
		self.rect.set_name("rectitem")
		
		self.spinbox_left = SpinBoxField(self)
		self.spinbox_left.set_name("left")
		
		self.spinbox_top = SpinBoxField(self)
		self.spinbox_top.set_name("top")
		
		self.spinbox_right = SpinBoxField(self)
		self.spinbox_right.set_name("right")
		
		self.spinbox_bottom = SpinBoxField(self)
		...
		self.button_open = ButtonOpenFile(self.manipulation, self)
		self.button_open.setText(internationalization["button_open"])
		
		self.button_print = ButtonPrint(self.manipulation, self)
		self.button_print.setText(internationalization["button_print"])
		
		self.scale = QLabel(internationalization["scale"], self)
		self.lcd = QLCDNumber(self)
		self.slider = Slider(self.lcd, self)		
		
		
		###### добавление  к виджетам других виджетов #######################
		
		self.scene.add_objects(self.manipulation, self.pixmap)
		self.button_open.add_objects(self.pixmap)
		...
		#self.layout_paper.setStretch(1,0)		
		...
		self.scale_slider_layout.addWidget(self.lcd)
		...
		self.scale_slider_layout.setStretch(3,1)
